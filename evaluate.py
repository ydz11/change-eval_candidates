...
def build_eval_candidates(eval_user_item_pairs, train_df, num_neg=100, seed=42):
    ...
    for idx, (u, pos_i) in enumerate(zip(users.tolist(), pos_items.tolist())):
        pool = train_df[(train_df["user_id"] == u) & (train_df["rating"] < 4)]["item_id"].values
        pool = pool[pool != pos_i]
...
def evaluate_model(model, eval_users, eval_candidates, k, device, batch_size=4096):
    ...
    for u, items in dl:
        ...
        for b in range(bsz):
            row_scores = scores[b]          # predicted scores
            pos_score  = row_scores[0]      # positive item score
            neg_scores = row_scores[1:]     # negative item scores

            # Predict rank: rank of positive among all candidates
            rank = 1 + int((neg_scores > pos_score).sum())

            # HR@k
            hit = 1.0 if rank <= k else 0.0

            # NDCG@k: real_rank=[1,0,0,...], predict_rank按rank位置放1
            ndcg = ndcg_from_rank(rank, k)
